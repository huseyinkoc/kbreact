...
import { Link, useLocation } from 'react-router-dom'; // ✅ Next.js yerine React Router kullanıldı
...
function CollapsibleMenuItem({ item }: { item: ItemType }) {
  ...
  const isActive = item.subMenuItems?.some(
    (subMenuItem) => subMenuItem.href === location.pathname
  );
  ...
  return (
    ...
    >
      {item?.subMenuItems?.map((subMenuItem, index) => {
        const isChildActive = location.pathname === (subMenuItem?.href as string);
